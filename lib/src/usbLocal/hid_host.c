...
{
    hid_iface_t *hid_iface = (hid_iface_t*)calloc(1, sizeof(hid_iface_t));
...
static esp_err_t hid_class_request_report_descriptor(hid_iface_t *iface)
{
    ...
    iface->report_desc = (u_int8_t *)malloc(iface->report_desc_size);
                                  ...
                                  hid_device_t **hid_device_handle)
{
    ...
    HID_GOTO_ON_FALSE( hid_device = (hid_device_t *)calloc(1, sizeof(hid_device_t)),
                       ESP_ERR_NO_MEM,
...
esp_err_t hid_host_install(const hid_host_driver_config_t *config)
{
    ...
    hid_driver_t *driver = (hid_driver_t *)heap_caps_calloc(1, sizeof(hid_driver_t), MALLOC_CAP_DEFAULT);
