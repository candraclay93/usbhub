...
#include "./usbLocal/hid_host.h"
#include "usbLocal/hid_usage_keyboard.h"
#include "usbLocal/hid_usage_mouse.h"
                             ...
                             unsigned int length)
{
  ...
  return false; //::cleieclient_event_callback
}
                                 ...
                                 void *arg)
{
  ...
  case HID_HOST_INTERFACE_EVENT_DISCONNECTED:
    log_v( "HID Device, protocol '%s' DISCONNECTED",
             hid_proto_name_str[dev_params.proto]);
    ESP_ERROR_CHECK(hid_host_device_close(hid_device_handle));
    break;
  case HID_HOST_INTERFACE_EVENT_TRANSFER_ERROR:
    log_v( "HID Device, protocol '%s' TRANSFER_ERROR",
             hid_proto_name_str[dev_params.proto]);
    break;
  default:
                           ...
                           void *arg)
{
  ...
  {
    log_v( "HID Device, protocol '%s' CONNECTED",
             hid_proto_name_str[dev_params.proto]);
...
static void usb_lib_task(void *arg)
{
  ...
  while (gpio_get_level(APP_QUIT_PIN) != 0)
  {
    ...
    if (event_flags & USB_HOST_LIB_EVENT_FLAGS_NO_CLIENTS)
    {
      ...
      log_v( "USB Event flags: NO_CLIENTS");
    }
    // All devices were removed
    if (event_flags & USB_HOST_LIB_EVENT_FLAGS_ALL_FREE)
    {
      log_v( "USB Event flags: ALL_FREE");
    }
  }
